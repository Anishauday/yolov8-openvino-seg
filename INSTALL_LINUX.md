...
- **Line endings (CRLF):** If you get "bad interpreter" or "command not found", the script may have Windows line endings. Run before executing:
  ```bash
  sed -i 's/\r$//' install_linux.sh
  ```
  (Or `dos2unix install_linux.sh`). The installer also tries to self-fix when run via `bash install_linux.sh`.
