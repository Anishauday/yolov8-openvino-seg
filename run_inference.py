...
Examples:
  ...
  python run_inference.py -d CPU "Waves"           # CPU (recommended if GPU has igc_check error)
  python run_inference.py --width 704 --height 480 "River scene"  # Higher resolution
  python run_inference.py --int8 "Waves on a beach"
    ...
    parser.add_argument(
        ...
        help="Device (default: CPU; use CPU to avoid Intel GPU igc_check errors)",
    )
    parser.add_argument(
        "--height", type=int, default=384,
        help="Video height, divisible by 32 (default: 384; use 384 to avoid GPU issues)",
    )
    parser.add_argument(
        "--width", type=int, default=384,
        help="Video width, divisible by 32 (default: 384)",
    )
    parser.add_argument(
        "--frames", type=int, default=17,
        help="Number of frames, e.g. 9, 17, 25 (default: 17)",
    )
    parser.add_argument(
    ...
    # Validate resolution (LTX requires divisible by 32)
    if args.height % 32 != 0 or args.width % 32 != 0:
        print("ERROR: --height and --width must be divisible by 32")
        sys.exit(1)
    ...
    print(f"  >> {args.width}x{args.height} | {args.frames} frames | {args.steps} steps | {args.device}")
    print("")
