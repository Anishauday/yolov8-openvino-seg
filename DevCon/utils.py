...
def _gst_prop_value(path: str) -> str:
    """Format paths for GstParse (gst-launch). Always quote and use POSIX slashes.

    Unquoted Windows paths (``C:\\Users\\...``) confuse the pipeline parser (backslash escapes),
    which yields **WARNING: erroneous pipeline: syntax error** on Windows notebooks.
    """
...
def run_visual(pipeline: str) -> None:
    ...
    # Do not use capture_output=True here: long runs can fill pipes and deadlock; match V1 bash behavior.
    r = subprocess.run(
...
def run_benchmark(pipeline: str) -> None:
    ...
    if r.returncode != 0:
        raise RuntimeError(f"gst-launch failed with exit code {r.returncode}")
